...
public class StudentDaoImpl implements StudentDao {
    ...
    private final static String QUERY_CREATE = "insert into student(id, groups_id, first_name, last_name, age) values (?, ?, ?, ?, ?)";
    private final static String QUERY_GET_BY_ID = "select * from student where id = ?";
    private final static String QUERY_UPDATE = "update student set groups_id = ?, first_name = ?, last_name = ?, age = ? where id = ?";
    ...
    public void update(Student student, int id) {
        ...
        jdbcTemplate.update(QUERY_UPDATE, student.getGroupsId(), student.getFirstName(),
                student.getLastName(), student.getAge(), id);
        LOGGER.trace("Exit the method");
    }

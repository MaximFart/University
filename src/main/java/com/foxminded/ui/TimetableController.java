...
//        model.addAttribute("timetables", timetableDaoService.findAll().stream().map(Optional::get).collect(Collectors.toList()));
//        return "timetable/timetables";
//    }
//
//    @GetMapping("/{id}")
//    public String getById(@PathVariable("id") int id, Model model) throws Exception {
//        model.addAttribute("timetable", timetableDaoService.getById(id).get());
...
//
//    @PatchMapping("/{id}")
//    public String update(@ModelAttribute("timetable") Timetable timetable, @PathVariable("id") int id) throws UserInputException {
